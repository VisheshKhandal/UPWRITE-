...
import { Bell, HelpCircle, Info, LogOut, Moon, PenLine, Search, Settings, Sun, UserRound, X } from "lucide-react";
...
export const TopBar = () => {
  ...
  const menuItems = user
    ? [
        [
          { label: "Profile", icon: UserRound, action: () => go(`/profile/${user.username}`), tour: "profile" }
        ],
        [
          { label: "Studio", icon: UserRound, action: () => go("/studio") },
          { label: "Notifications", icon: Bell, action: () => go("/notifications"), tour: "notifications" },
          { label: "Settings", icon: Settings, action: () => go("/settings/profile"), tour: "settings" }
        ],
        [
          { label: "About Upwrite", icon: Info, action: () => go("/about") },
          { label: "Help Center", icon: HelpCircle, action: () => go("/help") }
        ]
      ]
    : [];
    ...
    <>
      ...
      <div className="mt-2 grid gap-1">
        {menuItems.map((group, groupIndex) => (
          <div key={groupIndex} className={cn(groupIndex > 0 && "border-t border-ink-200/70 pt-1.5 dark:border-ink-800/80", groupIndex < menuItems.length - 1 && "pb-1.5")}>
            {group.map((item) => (
              <button key={item.label} type="button" data-tour={item.tour} onClick={item.action} className="flex min-h-11 w-full items-center gap-3 rounded-lg px-3 text-left text-sm font-medium text-ink-700 transition-[background-color,color,transform] duration-150 hover:bg-ink-100 hover:text-ink-950 focus-visible:bg-accent-50 focus-visible:text-ink-950 dark:text-ink-200 dark:hover:bg-ink-900 dark:hover:text-ink-50 dark:focus-visible:bg-accent-950/30 dark:focus-visible:text-ink-50">
        ...
        <div className="border-t border-ink-200/70 pt-1.5 dark:border-ink-800/80">
        <button type="button" onClick={() => dispatch(toggleTheme())} className="flex min-h-11 w-full items-center gap-3 rounded-lg px-3 text-left text-sm font-medium text-ink-700 transition-[background-color,color] duration-150 hover:bg-ink-100 hover:text-ink-950 focus-visible:bg-accent-50 dark:text-ink-200 dark:hover:bg-ink-900 dark:hover:text-ink-50 dark:focus-visible:bg-accent-950/30">
          {theme === "dark" ? <Sun className="h-4 w-4 text-ink-500" /> : <Moon className="h-4 w-4 text-ink-500" />}
          {theme === "dark" ? "Light mode" : "Dark mode"}
        </button>
        <button type="button" onClick={onLogout} disabled={logoutState.isLoading} className="mt-1 flex min-h-11 w-full items-center gap-3 rounded-lg px-3 text-left text-sm font-semibold text-red-600 transition-[background-color,color] duration-150 hover:bg-red-50 disabled:opacity-60 dark:hover:bg-red-950/20">
  ...
  return (
    <>
      <header className="sticky top-0 z-20 border-b border-ink-200 bg-ink-50/90 backdrop-blur dark:border-ink-800 dark:bg-ink-950/90">
        <div className="mx-auto flex h-16 max-w-[88rem] items-center justify-between gap-3 px-4 transition-[max-width] duration-300 ease-out sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-2 md:ml-auto">
            <Button variant="secondary" size="icon" className="md:hidden" onClick={() => navigate(user ? "/read" : "/search")} aria-label="Search"><Search className="h-4 w-4" /></Button>
